...
async function getStats() {
  ...
  // For active properties, compare to total
  ...
  return {
    ...
    changes: {
      ...
      leads: leadsChange,
      visits: { value: visitsLastWeek || 0, type: 'neutral' as const },
    }
  };
}
...
async function getRecentLeads() {
  ...
  const { data: leads } = await supabase
    ...
    .select(`
      *,
      properties:property_id (title, reference)
    `)
    .order('created_at', { ascending: false })
    .limit(5);
...
export default async function AdminDashboard() {
  const [stats, recentActivity, recentLeads] = await Promise.all([
    getStats(),
    getRecentActivity(),
    getRecentLeads(),
  ]);
